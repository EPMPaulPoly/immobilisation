...
def inventory_duplicates_agg_function(x:pd.DataFrame)->pd.Series:
    d = {}
...
def merge_lot_data(inventory:PI.ParkingInventory)->PI.ParkingInventory:
    '''
    #merge_lot_data
    ...
    if len(lots_list_to_purge_from_self)>0:
        aggregate_parking_data = lots_to_clean_up.groupby(
                [config_db.db_column_lot_id]
            ).apply(
                inventory_duplicates_agg_function, include_groups=True
            ).reset_index()
