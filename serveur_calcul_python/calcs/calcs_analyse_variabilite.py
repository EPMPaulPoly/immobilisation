...
import db_interface.db_inventory as DBI
from db_interface import db_tax_dataset as DBTD
from calcs import calcs_mins_from_inputs as CMFI
from calcs import calcs_mins_reg_sets as CMRS
from db_interface import db_parking_reg_sets as DBPRS
def analyse_variabilite(engine:Engine,scales:list[float]=None)->bool:
    ...
    tax_dataset,lot_land_use_and_validity = DBTD.get_all_lots_with_valid_data(engine=engine)
    ...
    inventory_data = DBI.get_lot_data_by_estimation(valid_lot_list,2) # Obtiens les données calculées
    ...
    reg_sets = DBPRS.get_all_reg_sets_from_database(engine=engine)
    ...
    for scale in scales:
        for reg_set in reg_sets:
            ...
            parking_inventory_indiv_reg_set =  CMRS.calculate_parking_specific_reg_set(reg_set,tax_data_set_final,scale=scale)
            # calcul de l'inventaire
            aggregate_data = IA.aggregate_statistics_by_land_use(parking_inventory_indiv_reg_set,lot_uses=lot_list_final,level=1)
