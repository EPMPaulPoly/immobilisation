...
def generate_values_based_on_available_data(entree:dict)->PCI.ParkingCalculationInputs:
    ...
    if reglements !=['0']:
        ...
        er_pert = DBPRS.from_sql([int(er) for er in ensembles_reglements])
        for i, id_er_pert in enumerate(ensembles_reglements):
            ...
            reglement:PR.ParkingRegulations = DBPR.from_postgis(reg_a_util)
            unites = reglement.reg_def[config_db.db_column_parking_unit_id].unique().tolist()
            if len(unites)>1 and unites[0]!= int(units):
                ValueError("L'option graphique ne supporte pas plus d'une unité à l'heure actuelle")
            df_reg_uni['unite'] = int(units)
...
def generate_calculation_input_from_tax_data(reg_to_calculate:PR.ParkingRegulations,tax_data: TD.TaxDataset)->PCI.ParkingCalculationInputs:
    if (reg_to_calculate.check_only_one_regulation()):
        ...
        print('test')
        output_pii = PCI.ParkingCalculationInputs(output_gb)
        return output_pii
    else:
